...
| `EventType` | `string` or enum | Required; allow-listed values such as `ProjectCreated`, `ProjectUpdated`, `ProjectDeleted`, `MilestoneCreated`, `MilestoneStatusUpdated`, `MilestoneDeleted`, and `MILESTONE_REOPENED`. |
...
| `ActorIpAddress` | `string?` | Nullable normalized IPv4 or IPv6 address resolved by the Project API server context. |
...
| `MILESTONE_REOPENED` | Milestone ID, project ID, actor, organization, actor IP when available, and full `Completed` to `InProgress` snapshots. | Distinct members of the milestone project's team, including the actor when they are a team member; message identifies the reopen action. |
